//
//  MockNetworkManager.swift
//  Sports_AppTests
//
//  Created by Ahmed Ashraf on 10/10/2023.
//

import Foundation
import Alamofire
@testable import Sports_App

class MockNetworkManager: Network{
    func getLeagues(chosen_sport: String, completionHandler: @escaping ([League]?, Error?) -> Void) {
        if shouldReturnError{
            completionHandler(nil, NetworkError.responseError)
        } else{
            do {
                self.encoding()
                let result = try JSONDecoder().decode(Leagues.self, from: leaguesData ?? Data())
                print("================")
                print("Debug\(result)")
                print("================")
                completionHandler(result.result, nil)
                
            } catch {
                completionHandler(nil, error)
            }
        }
    }
    
    func getTeams(LeagueId: Int, chosen_sport: String, completionHandler: @escaping ([Team]?, Error?) -> Void) {
        if shouldReturnError{
            completionHandler(nil, NetworkError.responseError)
        } else{
            do {
                self.encoding()
                let result = try JSONDecoder().decode(Teams.self, from: teamsData ?? Data())
                print("================")
                print("Debug\(result)")
                print("================")
                completionHandler(result.result, nil)
                
            } catch {
                completionHandler(nil, error)
            }
        }
    }
    
    func getLatestEvents(leagueId: Int, chosen_sport: String, completion: (([Event]?, Error?) -> Void)?) {
        if shouldReturnError{
            completion?(nil, NetworkError.responseError)
        } else{
            do {
                self.encoding()
                let result = try JSONDecoder().decode(Events.self, from: leaguesData ?? Data())
                print("================")
                print("Debug\(result)")
                print("================")
                completion?(result.result, nil)
                
            } catch {
                completion?(nil, error)
            }
        }
    }
    
    var eventsData: Data?
    var leaguesData: Data?
    var teamsData: Data?
    var shouldReturnError: Bool = false
    
    /*    init(shouldReturnError: Bool){
     self.shouldReturnError = shouldReturnError
     }
     */
    
    let eventJson = """
    {
        "success": 1,
        "result": [
            {
                "event_key": 1225882,
                "event_date": "2023-10-23",
                "event_time": "21:00",
                "event_home_team": "Tottenham Hotspur",
                "home_team_key": 164,
                "event_away_team": "Fulham",
                "away_team_key": 3085,
                "event_halftime_result": "",
                "event_final_result": "-",
                "event_ft_result": "",
                "event_penalty_result": "",
                "event_status": "",
                "country_name": "England",
                "league_name": "Premier League",
                "league_key": 152,
                "league_round": "Round 9",
                "league_season": "2023/2024",
                "event_live": "0",
                "event_stadium": "Tottenham Hotspur Stadium (London)",
                "event_referee": "",
                "home_team_logo": "https://apiv2.allsportsapi.com/logo/164_tottenham-hotspur.jpg",
                "away_team_logo": "https://apiv2.allsportsapi.com/logo/3085_fulham.jpg",
                "event_country_key": 44,
                "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/152_premier-league.png",
                "country_logo": "https://apiv2.allsportsapi.com/logo/logo_country/44_england.png",
                "event_home_formation": "",
                "event_away_formation": "",
                "fk_stage_key": 6,
                "stage_name": "Current",
                "league_group": null,
                "goalscorers": [],
                "substitutes": [],
                "cards": [],
                "vars": {
                    "home_team": [],
                    "away_team": []
                },
                "lineups": {
                    "home_team": {
                        "starting_lineups": [],
                        "substitutes": [],
                        "coaches": [],
                        "missing_players": []
                    },
                    "away_team": {
                        "starting_lineups": [],
                        "substitutes": [],
                        "coaches": [],
                        "missing_players": []
                    }
                },
                "statistics": []
            },
            {
                "event_key": 1225874,
                "event_date": "2023-10-22",
                "event_time": "17:30",
                "event_home_team": "Aston Villa",
                "home_team_key": 3088,
                "event_away_team": "West Ham United",
                "away_team_key": 3081,
                "event_halftime_result": "",
                "event_final_result": "-",
                "event_ft_result": "",
                "event_penalty_result": "",
                "event_status": "",
                "country_name": "England",
                "league_name": "Premier League",
                "league_key": 152,
                "league_round": "Round 9",
                "league_season": "2023/2024",
                "event_live": "0",
                "event_stadium": "Villa Park (Birmingham)",
                "event_referee": "",
                "home_team_logo": "https://apiv2.allsportsapi.com/logo/3088_aston-villa.jpg",
                "away_team_logo": "https://apiv2.allsportsapi.com/logo/3081_west-ham-united.jpg",
                "event_country_key": 44,
                "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/152_premier-league.png",
                "country_logo": "https://apiv2.allsportsapi.com/logo/logo_country/44_england.png",
                "event_home_formation": "",
                "event_away_formation": "",
                "fk_stage_key": 6,
                "stage_name": "Current",
                "league_group": null,
                "goalscorers": [],
                "substitutes": [],
                "cards": [],
                "vars": {
                    "home_team": [],
                    "away_team": []
                },
                "lineups": {
                    "home_team": {
                        "starting_lineups": [],
                        "substitutes": [],
                        "coaches": [],
                        "missing_players": []
                    },
                    "away_team": {
                        "starting_lineups": [],
                        "substitutes": [],
                        "coaches": [],
                        "missing_players": []
                    }
                },
                "statistics": []
            }
    ]
}
"""
    
    let leaguesJson = """
{
    "success": 1,
    "result": [
        {
            "league_key": 4,
            "league_name": "UEFA Europa League",
            "country_key": 1,
            "country_name": "eurocups",
            "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/",
            "country_logo": null
        },
        {
            "league_key": 1,
            "league_name": "UEFA European Championship",
            "country_key": 1,
            "country_name": "eurocups",
            "league_logo": null,
            "country_logo": null
        },
        {
            "league_key": 683,
            "league_name": "UEFA Europa Conference League",
            "country_key": 1,
            "country_name": "eurocups",
            "league_logo": null,
            "country_logo": null
        },
        {
            "league_key": 3,
            "league_name": "UEFA Champions League",
            "country_key": 1,
            "country_name": "eurocups",
            "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/3_uefa_champions_league.png",
            "country_logo": null
        },
        {
            "league_key": 633,
            "league_name": "UEFA Nations League",
            "country_key": 1,
            "country_name": "eurocups",
            "league_logo": null,
            "country_logo": null
        },
        {
            "league_key": 28,
            "league_name": "World Cup",
            "country_key": 8,
            "country_name": "Worldcup",
            "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/28_world-cup.png",
            "country_logo": "https://apiv2.allsportsapi.com/logo/logo_country/8_worldcup.png"
        },
        {
            "league_key": 152,
            "league_name": "Premier League",
            "country_key": 44,
            "country_name": "England",
            "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/152_premier-league.png",
            "country_logo": "https://apiv2.allsportsapi.com/logo/logo_country/44_england.png"
        },
        {
            "league_key": 302,
            "league_name": "La Liga",
            "country_key": 6,
            "country_name": "Spain",
            "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/302_la-liga.png",
            "country_logo": "https://apiv2.allsportsapi.com/logo/logo_country/6_spain.png"
        },
        {
            "league_key": 207,
            "league_name": "Serie A",
            "country_key": 5,
            "country_name": "Italy",
            "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/207_serie-a.png",
            "country_logo": "https://apiv2.allsportsapi.com/logo/logo_country/5_italy.png"
        },
        {
            "league_key": 175,
            "league_name": "Bundesliga",
            "country_key": 4,
            "country_name": "Germany",
            "league_logo": "https://apiv2.allsportsapi.com/logo/logo_leagues/175_bundesliga.png",
            "country_logo": "https://apiv2.allsportsapi.com/logo/logo_country/4_germany.png"
        }
    ]
}
"""

    let teamsJson = """
    {
        "success": 1,
        "result": [
            {
                "team_key": 4973,
                "team_name": "Torino",
                "team_logo": "https://apiv2.allsportsapi.com/logo/4973_torino.jpg",
                "players": [
                    {
                        "player_key": 2057771707,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/10626_a-tameze.jpg",
                        "player_name": "A. Tameze",
                        "player_number": "61",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "29",
                        "player_match_played": "5",
                        "player_goals": "0",
                        "player_yellow_cards": "2",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "3",
                        "player_substitutes_on_bench": "3",
                        "player_assists": "0",
                        "player_birthdate": "1994-02-04",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 3772917518,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/17071_k-djidji.jpg",
                        "player_name": "K. Djidji",
                        "player_number": "26",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "30",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "Yes",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "1992-11-30",
                        "player_is_captain": "0",
                        "player_shots_total": "1",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "3",
                        "player_tackles": "3",
                        "player_blocks": "1",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "7",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "16",
                        "player_duels_won": "10",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "108",
                        "player_passes_accuracy": "103",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": "6.60"
                    },
                    {
                        "player_key": 760652184,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/20493_k-linetty.jpg",
                        "player_name": "K. Linetty",
                        "player_number": "77",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "28",
                        "player_match_played": "5",
                        "player_goals": "0",
                        "player_yellow_cards": "1",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "1",
                        "player_substitutes_on_bench": "6",
                        "player_assists": "0",
                        "player_birthdate": "1995-02-02",
                        "player_is_captain": "0",
                        "player_shots_total": "1",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "3",
                        "player_tackles": "3",
                        "player_blocks": "2",
                        "player_crosses_total": "1",
                        "player_interceptions": "1",
                        "player_clearances": "2",
                        "player_dispossesed": "1",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "16",
                        "player_duels_won": "7",
                        "player_dribble_attempts": "3",
                        "player_dribble_succ": "1",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "29",
                        "player_passes_accuracy": "22",
                        "player_key_passes": "1",
                        "player_woordworks": "",
                        "player_rating": "6.70"
                    },
                    {
                        "player_key": 2599684352,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/25845_v-lazaro.jpg",
                        "player_name": "V. Lazaro",
                        "player_number": "20",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "27",
                        "player_match_played": "6",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "1",
                        "player_substitutes_on_bench": "2",
                        "player_assists": "0",
                        "player_birthdate": "1996-03-24",
                        "player_is_captain": "0",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "4",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "1",
                        "player_duels_won": "1",
                        "player_dribble_attempts": "1",
                        "player_dribble_succ": "1",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "27",
                        "player_passes_accuracy": "22",
                        "player_key_passes": "1",
                        "player_woordworks": "",
                        "player_rating": "6.95"
                    },
                    {
                        "player_key": 1285899697,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/29104_a-sanabria.jpg",
                        "player_name": "A. Sanabria",
                        "player_number": "9",
                        "player_country": null,
                        "player_type": "Forwards",
                        "player_age": "27",
                        "player_match_played": "4",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "2",
                        "player_substitutes_on_bench": "3",
                        "player_assists": "0",
                        "player_birthdate": "1996-03-04",
                        "player_is_captain": "0",
                        "player_shots_total": "3",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "3",
                        "player_tackles": "3",
                        "player_blocks": "",
                        "player_crosses_total": "1",
                        "player_interceptions": "",
                        "player_clearances": "1",
                        "player_dispossesed": "7",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "41",
                        "player_duels_won": "13",
                        "player_dribble_attempts": "2",
                        "player_dribble_succ": "1",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "67",
                        "player_passes_accuracy": "51",
                        "player_key_passes": "2",
                        "player_woordworks": "",
                        "player_rating": "6.60"
                    },
                    {
                        "player_key": 1886734424,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/30840_n-radonji.jpg",
                        "player_name": "N. Radonjić",
                        "player_number": "10",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "27",
                        "player_match_played": "7",
                        "player_goals": "3",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "4",
                        "player_substitutes_on_bench": "3",
                        "player_assists": "0",
                        "player_birthdate": "1996-02-15",
                        "player_is_captain": "0",
                        "player_shots_total": "5",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "",
                        "player_tackles": "1",
                        "player_blocks": "",
                        "player_crosses_total": "5",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "1",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "22",
                        "player_duels_won": "9",
                        "player_dribble_attempts": "17",
                        "player_dribble_succ": "6",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "51",
                        "player_passes_accuracy": "42",
                        "player_key_passes": "7",
                        "player_woordworks": "",
                        "player_rating": "7.55"
                    },
                    {
                        "player_key": 3036656066,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/41419_v-milinkovi-savi.jpg",
                        "player_name": "V. Milinković-Savić",
                        "player_number": "32",
                        "player_country": null,
                        "player_type": "Goalkeepers",
                        "player_age": "26",
                        "player_match_played": "7",
                        "player_goals": "0",
                        "player_yellow_cards": "1",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "1997-02-20",
                        "player_is_captain": "0",
                        "player_shots_total": "",
                        "player_goals_conceded": "2",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "3",
                        "player_dispossesed": "",
                        "player_saves": "22",
                        "player_inside_box_saves": "11",
                        "player_duels_total": "2",
                        "player_duels_won": "2",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "221",
                        "player_passes_accuracy": "114",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": "8.00"
                    },
                    {
                        "player_key": 2705223089,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/41667_n-vlai.jpg",
                        "player_name": "N. Vlašić",
                        "player_number": "16",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "26",
                        "player_match_played": "6",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "2",
                        "player_substitutes_on_bench": "2",
                        "player_assists": "0",
                        "player_birthdate": "1997-10-04",
                        "player_is_captain": "0",
                        "player_shots_total": "1",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "1",
                        "player_tackles": "4",
                        "player_blocks": "",
                        "player_crosses_total": "3",
                        "player_interceptions": "",
                        "player_clearances": "2",
                        "player_dispossesed": "6",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "22",
                        "player_duels_won": "8",
                        "player_dribble_attempts": "1",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "81",
                        "player_passes_accuracy": "67",
                        "player_key_passes": "5",
                        "player_woordworks": "",
                        "player_rating": "7.03"
                    },
                    {
                        "player_key": 3127862871,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/45356_d-zapata.jpg",
                        "player_name": "D. Zapata",
                        "player_number": "91",
                        "player_country": null,
                        "player_type": "Forwards",
                        "player_age": "32",
                        "player_match_played": "5",
                        "player_goals": "1",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "3",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "1991-04-01",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 1407422813,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/48830_m-vojvoda.jpg",
                        "player_name": "M. Vojvoda",
                        "player_number": "27",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "28",
                        "player_match_played": "3",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "1",
                        "player_assists": "0",
                        "player_birthdate": "1995-02-01",
                        "player_is_captain": "0",
                        "player_shots_total": "2",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "1",
                        "player_tackles": "5",
                        "player_blocks": "",
                        "player_crosses_total": "5",
                        "player_interceptions": "1",
                        "player_clearances": "3",
                        "player_dispossesed": "2",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "17",
                        "player_duels_won": "10",
                        "player_dribble_attempts": "3",
                        "player_dribble_succ": "2",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "116",
                        "player_passes_accuracy": "85",
                        "player_key_passes": "3",
                        "player_woordworks": "",
                        "player_rating": "6.70"
                    },
                    {
                        "player_key": 2807606933,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/53332_y-karamoh.jpg",
                        "player_name": "Y. Karamoh",
                        "player_number": "7",
                        "player_country": null,
                        "player_type": "Forwards",
                        "player_age": "25",
                        "player_match_played": "5",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "1",
                        "player_substitutes_on_bench": "6",
                        "player_assists": "0",
                        "player_birthdate": "1998-07-08",
                        "player_is_captain": "0",
                        "player_shots_total": "2",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "2",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "2",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "11",
                        "player_duels_won": "4",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "8",
                        "player_passes_accuracy": "5",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": "6.70"
                    },
                    {
                        "player_key": 4280075590,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/63912_r-bellanova.jpg",
                        "player_name": "R. Bellanova",
                        "player_number": "19",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "23",
                        "player_match_played": "7",
                        "player_goals": "0",
                        "player_yellow_cards": "2",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "5",
                        "player_substitutes_on_bench": "1",
                        "player_assists": "1",
                        "player_birthdate": "2000-05-17",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 2912454414,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/66666_p-schuurs.jpg",
                        "player_name": "P. Schuurs",
                        "player_number": "3",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "23",
                        "player_match_played": "7",
                        "player_goals": "1",
                        "player_yellow_cards": "2",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "1999-11-26",
                        "player_is_captain": "0",
                        "player_shots_total": "2",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "5",
                        "player_tackles": "4",
                        "player_blocks": "3",
                        "player_crosses_total": "",
                        "player_interceptions": "3",
                        "player_clearances": "17",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "27",
                        "player_duels_won": "15",
                        "player_dribble_attempts": "2",
                        "player_dribble_succ": "1",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "119",
                        "player_passes_accuracy": "101",
                        "player_key_passes": "2",
                        "player_woordworks": "",
                        "player_rating": "7.50"
                    },
                    {
                        "player_key": 3833242469,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/76806_i-ili.jpg",
                        "player_name": "I. Ilić",
                        "player_number": "8",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "22",
                        "player_match_played": "6",
                        "player_goals": "0",
                        "player_yellow_cards": "1",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "2",
                        "player_substitutes_on_bench": "2",
                        "player_assists": "2",
                        "player_birthdate": "2001-03-17",
                        "player_is_captain": "0",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "1",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "1",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "3",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "17",
                        "player_passes_accuracy": "16",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": "6.20"
                    },
                    {
                        "player_key": 4242646149,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/76891_p-pellegri.jpg",
                        "player_name": "P. Pellegri",
                        "player_number": "11",
                        "player_country": null,
                        "player_type": "Forwards",
                        "player_age": "22",
                        "player_match_played": "6",
                        "player_goals": "0",
                        "player_yellow_cards": "1",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "6",
                        "player_assists": "0",
                        "player_birthdate": "2001-03-17",
                        "player_is_captain": "0",
                        "player_shots_total": "2",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "4",
                        "player_tackles": "1",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "3",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "13",
                        "player_duels_won": "3",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "14",
                        "player_passes_accuracy": "10",
                        "player_key_passes": "1",
                        "player_woordworks": "",
                        "player_rating": "7.25"
                    },
                    {
                        "player_key": 3554993026,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/78882_a-buongiorno.jpg",
                        "player_name": "A. Buongiorno",
                        "player_number": "4",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "24",
                        "player_match_played": "6",
                        "player_goals": "1",
                        "player_yellow_cards": "1",
                        "player_red_cards": "0",
                        "player_injured": "Yes",
                        "player_substitute_out": "1",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "1999-06-06",
                        "player_is_captain": "1",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "5",
                        "player_tackles": "6",
                        "player_blocks": "6",
                        "player_crosses_total": "",
                        "player_interceptions": "12",
                        "player_clearances": "9",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "42",
                        "player_duels_won": "24",
                        "player_dribble_attempts": "1",
                        "player_dribble_succ": "1",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "171",
                        "player_passes_accuracy": "144",
                        "player_key_passes": "1",
                        "player_woordworks": "",
                        "player_rating": "7.22"
                    },
                    {
                        "player_key": 2547722931,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/80446_l-gemello.jpg",
                        "player_name": "L. Gemello",
                        "player_number": "1",
                        "player_country": null,
                        "player_type": "Goalkeepers",
                        "player_age": "23",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "7",
                        "player_assists": "0",
                        "player_birthdate": "2000-07-03",
                        "player_is_captain": "0",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 3538723488,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/96370_s-ricci.jpg",
                        "player_name": "S. Ricci",
                        "player_number": "28",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "22",
                        "player_match_played": "6",
                        "player_goals": "0",
                        "player_yellow_cards": "1",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "2",
                        "player_substitutes_on_bench": "1",
                        "player_assists": "1",
                        "player_birthdate": "2001-08-21",
                        "player_is_captain": "0",
                        "player_shots_total": "2",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "4",
                        "player_tackles": "6",
                        "player_blocks": "1",
                        "player_crosses_total": "4",
                        "player_interceptions": "5",
                        "player_clearances": "6",
                        "player_dispossesed": "4",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "29",
                        "player_duels_won": "16",
                        "player_dribble_attempts": "4",
                        "player_dribble_succ": "1",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "202",
                        "player_passes_accuracy": "175",
                        "player_key_passes": "1",
                        "player_woordworks": "",
                        "player_rating": "7.02"
                    },
                    {
                        "player_key": 3329368140,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/123525_m-popa.jpg",
                        "player_name": "M. Popa",
                        "player_number": "71",
                        "player_country": null,
                        "player_type": "Goalkeepers",
                        "player_age": "22",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "3",
                        "player_assists": "0",
                        "player_birthdate": "2000-10-12",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 94031944,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/101246_d-zima.jpg",
                        "player_name": "D. Zima",
                        "player_number": "6",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "22",
                        "player_match_played": "1",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "4",
                        "player_assists": "0",
                        "player_birthdate": "2000-11-08",
                        "player_is_captain": "0",
                        "player_shots_total": "1",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "2",
                        "player_tackles": "2",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "6",
                        "player_clearances": "12",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "12",
                        "player_duels_won": "5",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "57",
                        "player_passes_accuracy": "50",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": "7.40"
                    },
                    {
                        "player_key": 3667547319,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/101600_b-soppy.jpg",
                        "player_name": "B. Soppy",
                        "player_number": "93",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "21",
                        "player_match_played": "3",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "1",
                        "player_substitutes_on_bench": "4",
                        "player_assists": "0",
                        "player_birthdate": "2002-02-21",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 4199068567,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/113347_s-sazonov.jpg",
                        "player_name": "S. Sazonov",
                        "player_number": "15",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "21",
                        "player_match_played": "2",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "1",
                        "player_substitutes_on_bench": "4",
                        "player_assists": "0",
                        "player_birthdate": "2002-02-01",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 611304328,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/221161_a-naposguessan.jpg",
                        "player_name": "A. N'Guessan",
                        "player_number": "94",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "20",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "7",
                        "player_assists": "0",
                        "player_birthdate": "2003-09-01",
                        "player_is_captain": "0",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 1517926349,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/125405_d-seck.jpg",
                        "player_name": "D. Seck",
                        "player_number": "23",
                        "player_country": null,
                        "player_type": "Forwards",
                        "player_age": "22",
                        "player_match_played": "4",
                        "player_goals": "0",
                        "player_yellow_cards": "1",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "3",
                        "player_substitutes_on_bench": "2",
                        "player_assists": "0",
                        "player_birthdate": "2001-02-10",
                        "player_is_captain": "0",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "1",
                        "player_tackles": "1",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "7",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "18",
                        "player_duels_won": "5",
                        "player_dribble_attempts": "7",
                        "player_dribble_succ": "4",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "56",
                        "player_passes_accuracy": "50",
                        "player_key_passes": "2",
                        "player_woordworks": "",
                        "player_rating": "6.63"
                    },
                    {
                        "player_key": 1214491911,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/222188_g-gineitis.jpg",
                        "player_name": "G. Gineitis",
                        "player_number": "66",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "19",
                        "player_match_played": "1",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "7",
                        "player_assists": "0",
                        "player_birthdate": "2004-04-15",
                        "player_is_captain": "0",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 88976424,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/189125_a-dellavalle.jpg",
                        "player_name": "A. Dellavalle",
                        "player_number": "25",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "19",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "2004-05-11",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 419287432,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/189126_j-antolini.jpg",
                        "player_name": "J. Antolini",
                        "player_number": "84",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "19",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "3",
                        "player_assists": "0",
                        "player_birthdate": "2004-01-10",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 3146839295,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/225967_a-ciammaglichella.jpg",
                        "player_name": "A. Ciammaglichella",
                        "player_number": "81",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "18",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "2005-01-26",
                        "player_is_captain": "0",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 813340885,
                        "player_image": null,
                        "player_name": "Z. Savva",
                        "player_number": "79",
                        "player_country": null,
                        "player_type": "Forwards",
                        "player_age": "17",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "2005-11-26",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 1323273898,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/249822_m-brezzo.jpg",
                        "player_name": "M. Brezzo",
                        "player_number": "22",
                        "player_country": null,
                        "player_type": "Goalkeepers",
                        "player_age": "18",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "4",
                        "player_assists": "0",
                        "player_birthdate": "2005-06-13",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 3591681319,
                        "player_image": null,
                        "player_name": "A. Njie",
                        "player_number": "29",
                        "player_country": null,
                        "player_type": "Forwards",
                        "player_age": "18",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "2005-05-14",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 2747102977,
                        "player_image": null,
                        "player_name": "Jonathan Silva",
                        "player_number": "30",
                        "player_country": null,
                        "player_type": "Midfielders",
                        "player_age": "19",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "2004-04-24",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 3441321725,
                        "player_image": null,
                        "player_name": "F. Desole",
                        "player_number": "56",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "17",
                        "player_match_played": "0",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "0",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "2006-01-20",
                        "player_is_captain": "",
                        "player_shots_total": "",
                        "player_goals_conceded": "",
                        "player_fouls_committed": "",
                        "player_tackles": "",
                        "player_blocks": "",
                        "player_crosses_total": "",
                        "player_interceptions": "",
                        "player_clearances": "",
                        "player_dispossesed": "",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "",
                        "player_duels_won": "",
                        "player_dribble_attempts": "",
                        "player_dribble_succ": "",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "",
                        "player_pen_missed": "",
                        "player_passes": "",
                        "player_passes_accuracy": "",
                        "player_key_passes": "",
                        "player_woordworks": "",
                        "player_rating": ""
                    },
                    {
                        "player_key": 989593305,
                        "player_image": "https://apiv2.allsportsapi.com/logo/players/117314_r-rodriguez.jpg",
                        "player_name": "R. Rodríguez",
                        "player_number": "13",
                        "player_country": null,
                        "player_type": "Defenders",
                        "player_age": "31",
                        "player_match_played": "7",
                        "player_goals": "0",
                        "player_yellow_cards": "0",
                        "player_red_cards": "0",
                        "player_injured": "No",
                        "player_substitute_out": "1",
                        "player_substitutes_on_bench": "0",
                        "player_assists": "0",
                        "player_birthdate": "1992-08-25",
                        "player_is_captain": "2",
                        "player_shots_total": "",
                        "player_goals_conceded": "0",
                        "player_fouls_committed": "",
                        "player_tackles": "12",
                        "player_blocks": "",
                        "player_crosses_total": "15",
                        "player_interceptions": "3",
                        "player_clearances": "6",
                        "player_dispossesed": "2",
                        "player_saves": "",
                        "player_inside_box_saves": "",
                        "player_duels_total": "25",
                        "player_duels_won": "21",
                        "player_dribble_attempts": "1",
                        "player_dribble_succ": "1",
                        "player_pen_comm": "",
                        "player_pen_won": "",
                        "player_pen_scored": "0",
                        "player_pen_missed": "0",
                        "player_passes": "156",
                        "player_passes_accuracy": "131",
                        "player_key_passes": "5",
                        "player_woordworks": "",
                        "player_rating": "7.60"
                    }
                ],
                "coaches": [
                    {
                        "coach_name": "I. Juric",
                        "coach_country": null,
                        "coach_age": null
                    }
                ]
            }
        ]
    }
"""
}


extension MockNetworkManager {
    
    func encoding(){
        eventsData = Data(eventJson.utf8)
        leaguesData = Data(leaguesJson.utf8)
        teamsData = Data(teamsJson.utf8)
    }
    
    func getEvents(leagueId: Int , chosen_sport : String, completion:(([Event]?, Error?) -> Void)?) {
        if shouldReturnError{
            completion? (nil, NetworkError.responseError)
        } else{
            do {
                self.encoding()
                let result = try JSONDecoder().decode(Events.self, from: eventsData ?? Data())
                print("================")
                print("Debug\(result)")
                print("================")
                completion? (result.result, nil)
                
            } catch {
                completion? (nil, error)
            }
        }
    }
}

enum NetworkError: Error{
    case responseError
}
